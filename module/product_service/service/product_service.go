...
func (service *productService) Create(ctx context.Context, companyID int64, req product_dto.Create) (int64, error) {
	...
	err := service.db.QueryRow(ctx, `
		INSERT INTO products (name, description, price, category_id, is_active, company_id)
		VALUES ($1, $2, $3, $4, COALESCE($5, true), $6)
		RETURNING id
	`, req.Name, req.Description, req.Price, req.CategoryID, req.IsActive, companyID).Scan(&id)
	{
		if err != nil {
			return 0, err
		}
...
func (service *productService) Update(ctx context.Context, companyID, id int64, req product_dto.Update) (int64, error) {
	...
	err := service.db.QueryRow(ctx, `
		...
		RETURNING id
	`, req.Name, req.Description, req.Price, req.CategoryID, req.IsActive, id, companyID).Scan(&updatedID)
	{
		if err != nil {
			return 0, err
		}
